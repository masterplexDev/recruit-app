...
public class CssExtractorFromJsp {
    ...
    public static void main(String[] args) {
        String jspFileUrl = "http://localhost/recruit-app/main/main.jsp";  // JSP 파일 URL (절대 경로 대입)
        String outputDirectory = "C:/dev/recruit-app/src/main/webapp/assets/css/main";// 아웃풋 파일 저장 경로 (수정 필요)
        String updatedJspFilePath = "C:/dev/recruit-app/src/main/webapp/main/main_updated.jsp"; // 업데이트 파일 저장 경로+ 저장명 지정 (수정 필요)
        String baseUrl = "http://localhost/recruit-app/assets/css/main/"; //CSS 파일내부 리소스 파일의 상대 경로를 절대 경로로 변경할 때 사용되는 기본 URL
        //(CSS 파일 저장 경로와 같아야 함)
